...
option=changeuni(request.getParameter("option"));
search=changeuni(request.getParameter("search"));
...
<br />
 ...
 <table style="width=1001px; height=1001px;" border="0" align="center" cellpadding="0" cellspacing="0" id="Table_01">
    <form action="<%=request.getContextPath()%>/project/notsearch.jsp" method="post">
    <tr>
...
try {
    ...
    String totalSql="SELECT COUNT(*) FROM notice WHERE "+option+"='"+search+"'";
...
<%    
    int pagePerRow = 10;
    //String listSql = "SELECT * FROM notice ORDER BY `¹øÈ£` LIMIT ?, ?";
    String listSql="SELECT * FROM notice WHERE "+option+"='"+search+"' order by `¹øÈ£` LIMIT ?,?";
...
%>
                    ...
                    <td padding="1"><a href='notcheck.jsp?abc=<%=listResultSet.getInt(1)%>'><%=listResultSet.getString(2)%></a></td>
...
%>
            <!-- <a href='notsearch.jsp?currentPage=</a> -->
<%
		//}
      // }
%>
